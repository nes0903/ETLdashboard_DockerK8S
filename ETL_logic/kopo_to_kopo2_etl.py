...
log_file = os.path.join(log_dir, f"local_to_kopo2_etl_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
try:
    ...
    # 로컬 kopo2 데이터베이스 연결
    kopo2_connection = mysql.connector.connect(**kopo2_DB_CONFIG)
    kopo2_cursor = kopo2_connection.cursor()
    logger.info("로컬 kopo2 데이터베이스 연결 성공")
    
    # 2. 이력 테이블 생성 (kopo2 DB에)
    ...
    """
    kopo2_cursor.execute(create_history_table_query)
    ...
    for table_name, url in EXCEL_FILES.items():
        ...
        # 테이블이 이미 존재하면 삭제
        local_cursor.execute(f"DROP TABLE IF EXISTS kopo.{table_name}")
        ...
        insert_query = f"""
        INSERT INTO kopo.{table_name} ({', '.join(columns)})
    ...
    # 4. 통합 테이블 생성 (kopo2 DB에)
    # 먼저 kopo2 데이터베이스에 필요한 테이블들을 복사
    kopo2_cursor.execute("USE kopo2")
    
    # 테이블이 이미 존재하면 삭제
    kopo2_cursor.execute("DROP TABLE IF EXISTS kopo2.위해상품")
    kopo2_cursor.execute("DROP TABLE IF EXISTS kopo2.위해상품부적합검사")
    kopo2_cursor.execute("DROP TABLE IF EXISTS kopo2.위해상품업체")
    kopo2_cursor.execute("DROP TABLE IF EXISTS kopo2.위해상품_위험도_분석")
    kopo2_connection.commit()
    logger.info("기존 테이블 삭제 완료")
    
    # kopo 데이터베이스의 테이블 구조를 가져와서 kopo2에 생성
    local_cursor.execute("USE kopo")
    
    # 위해상품 테이블 생성
    local_cursor.execute("SHOW CREATE TABLE 위해상품")
    create_table_query = local_cursor.fetchone()[1]
    kopo2_cursor.execute(create_table_query)
    kopo2_connection.commit()
    logger.info("위해상품 테이블 생성 완료")
    
    # 위해상품부적합검사 테이블 생성
    local_cursor.execute("SHOW CREATE TABLE 위해상품부적합검사")
    create_table_query = local_cursor.fetchone()[1]
    kopo2_cursor.execute(create_table_query)
    kopo2_connection.commit()
    logger.info("위해상품부적합검사 테이블 생성 완료")
    
    # 위해상품업체 테이블 생성
    local_cursor.execute("SHOW CREATE TABLE 위해상품업체")
    create_table_query = local_cursor.fetchone()[1]
    kopo2_cursor.execute(create_table_query)
    kopo2_connection.commit()
    logger.info("위해상품업체 테이블 생성 완료")
    
    # 데이터 복사
    kopo2_cursor.execute("""
        INSERT INTO kopo2.위해상품 
        SELECT * FROM kopo.위해상품
    """)
    kopo2_connection.commit()
    logger.info("위해상품 데이터 복사 완료")
    
    kopo2_cursor.execute("""
        INSERT INTO kopo2.위해상품부적합검사 
        SELECT * FROM kopo.위해상품부적합검사
    """)
    kopo2_connection.commit()
    logger.info("위해상품부적합검사 데이터 복사 완료")
    
    kopo2_cursor.execute("""
        INSERT INTO kopo2.위해상품업체 
        SELECT * FROM kopo.위해상품업체
    """)
    kopo2_connection.commit()
    logger.info("위해상품업체 데이터 복사 완료")
    ...
    kopo2_cursor.execute(create_joined_table_query)
    kopo2_connection.commit()
    logger.info("위해상품_위험도_분석 테이블 생성 완료")
    
    # 5. 데이터셋 테이블 생성 (kopo2 DB에)
    ...
    """
    kopo2_cursor.execute(create_dataset_table_query)
    logger.info("kopo2 DB에 데이터셋 테이블 생성 완료")
    ...
    for row in rows:
        ...
        # 데이터 저장
        kopo2_cursor.execute(insert_query, (json.dumps(input_data, ensure_ascii=False), output_text))
    
    kopo2_connection.commit()
    logger.info(f"총 {len(rows)}개의 데이터셋 생성 완료")
    
    # 7. 성공 이력 기록 (kopo2 DB에)
    insert_history_query = """
        INSERT INTO 데이터셋_최신화_이력 
        (process_name, status, error_message)
        VALUES (%s, %s, %s)
    """
    kopo2_cursor.execute(insert_history_query, ("ETL 프로세스", "성공", None))
    kopo2_connection.commit()
    ...
    try:
        insert_history_query = """
            ...
            VALUES (%s, %s, %s)
        """
        kopo2_cursor.execute(insert_history_query, ("ETL 프로세스", "실패", str(e)))
        kopo2_connection.commit()
    except:
        pass
